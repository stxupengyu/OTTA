...
def compute_micro_f1(
    ...
    Micro-F1 is the harmonic mean of global precision and recall computed
    from aggregated TP, FP, and FN across all samples.

    Args:
        predictions: List of predicted label lists per sample
        gold_labels: List of gold label lists per sample
    ...
    for pred, gold in zip(predictions, gold_labels):
        ...
        tp = len(pred_set.intersection(gold_set))  # Correctly predicted labels
        fp = len(pred_set - gold_set)  # Incorrectly predicted labels
        fn = len(gold_set - pred_set)  # Missed labels
...
def compute_macro_f1(
    ...
    Macro-F1 is the average F1 across labels.

    Args:
        predictions: List of predicted label lists per sample
        gold_labels: List of gold label lists per sample
        all_labels: All possible labels
    ...
    for label in all_labels:
        tp = 0  # Correctly predicted for this label
        fp = 0  # Incorrectly predicted for this label
        fn = 0  # Missed for this label
        ...
        # Precision/recall for this label
...
def compute_example_f1(
    ...
    Example-F1 is the average F1 across samples.
